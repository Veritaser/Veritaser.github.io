...
def save_checkpoint(step, epoch, model, optimizer, scaler, loss, checkpoint_type='regular'):
    ...
    if checkpoint_type == 'milestone':
        ckp_path = f'{args.save_dir}/sft_checkpoint_step{step+1}.pth'
    else:
        ckp_path = f'{args.save_dir}/sft_checkpoint_latest.pth'
        
    model_only_path = f'{args.save_dir}/sft_dim{lm_config.dim}_layers{lm_config.n_layers}_vocab_size{lm_config.vocab_size}.pth'
    ...
    Logger(f"SFT检查点已保存: {ckp_path}")
    model.train()


def load_checkpoint(checkpoint_path, model, optimizer, scaler):
    """从检查点恢复训练状态"""
    Logger(f"从SFT检查点恢复训练: {checkpoint_path}")
    ...
    Logger(f"恢复SFT训练状态 - 步数: {start_step}, Epoch: {start_epoch}, 上次损失: {last_loss:.6f}")
...
def find_latest_checkpoint(save_dir):
    ...
    checkpoint_pattern = os.path.join(save_dir, "sft_checkpoint_*.pth")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default="./train_3.5M_CN.json", help="训练数据路径")
