...
class ModelConfig(PretrainedConfig):
    ...
    def __init__(
            ...
            num_experts_per_tok: int = 2,   # 每个token激活的专家数量 (top-k)
            moe_freq: int = 2,             # 每隔多少层使用MoE (1表示每层都用)
...
class Router(nn.Module):
    """路由器 - 决定每个token应该分配给哪些专家"""
    def __init__(self, dim: int, num_experts: int):
        super().__init__()
        self.num_experts = num_experts
        self.router_weights = nn.Linear(dim, num_experts, bias=False)
    ...
    def forward(self, x):
        ...
        Returns:
            router_logits: 路由器输出的logits [batch_size, seq_len, num_experts]
        """
        # 计算路由权重
...
class MoELayer(nn.Module):
    ...
    def __init__(self, dim: int, hidden_dim: int, multiple_of: int, dropout: float, 
        ...
        self.experts = nn.ModuleList([
            Expert(dim, hidden_dim, multiple_of, dropout) 
    ...
    def forward(self, x):
        ...
        # 将输入重塑为 [batch_size * seq_len, dim] 以便处理
        x_flat = x.view(-1, dim)
        ...
        for i in range(self.num_experts_per_tok):
            ...
            expert_weights = top_k_probs[:, i]    # [batch_size * seq_len]
    ...
    def _compute_aux_loss(self, router_probs):
        ...
        # 理想情况下每个专家被选中的概率应该是1/num_experts
        uniform_prob = 1.0 / self.num_experts
        
        # 计算负载均衡损失 (变异系数的平方)
        aux_loss = torch.sum((expert_usage - uniform_prob) ** 2)
...
class DecoderLayer(nn.Module):
    def __init__(self, layer_id: int, args: ModelConfig):
        ...
        # 根据layer_id和moe_freq决定使用MLP还是MoE
        self.use_moe = (layer_id % args.moe_freq == 0) and (layer_id > 0)
    ...
    def forward(self, x, freqs_cos, freqs_sin):
        ...
        # 通过前馈神经网络（MLP或MoE）
        if self.use_moe:
            ffn_output, aux_loss = self.feed_forward.forward(self.ffn_norm(h))
            out = h + ffn_output
            return out, aux_loss
        else:
...
class Transformer(PreTrainedModel):
    ...
    def forward(self, tokens: torch.Tensor, targets: Optional[torch.Tensor] = None, loss_mask: Optional[torch.Tensor] = None, **kwargs) -> torch.Tensor:
        """
        - tokens: Optional[torch.Tensor], 输入 token 张量。
        - targets: Optional[torch.Tensor], 目标 token 张量。
        - loss_mask: Optional[torch.Tensor], 损失掩码张量。
        ...
        for layer in self.layers:
            if layer.use_moe:
                h, aux_loss = layer(h, freqs_cos, freqs_sin)
        ...
        if targets is not None:
            ...
            if loss_mask is not None:
                # 使用 reduction='none' 计算每个 token 的 loss
                loss_per_token = F.cross_entropy(logits.view(-1, logits.size(-1)), targets.view(-1), ignore_index=0, reduction='none')
                
                # 应用 mask
                mask_flat = loss_mask.view(-1).float()
                loss_sum = torch.sum(loss_per_token * mask_flat)
                mask_sum = mask_flat.sum()
                
                # 避免除以 0
                main_loss = loss_sum / (mask_sum + 1e-6)
            else:
                main_loss = F.cross_entropy(logits.view(-1, logits.size(-1)), targets.view(-1), ignore_index=0, reduction='mean')
            
            # 加入MoE辅助损失
            if num_moe_layers > 0:
                aux_loss = total_aux_loss / num_moe_layers
                self.last_loss = main_loss + self.args.aux_loss_coef * aux_loss
            else:
...
if __name__ == '__main__':
    ...
    # 统计MoE层数量
    moe_layers = sum(1 for layer in model.layers if layer.use_moe)
    print(f'MoE层数量：{moe_layers} / {len(model.layers)}')
    ...
    if output.last_loss is not None:
        print("Loss (包含MoE辅助损失):", output.last_loss.item())